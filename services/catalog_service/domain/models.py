"""Domain Models and Aggregate Root for Product Catalog Service."""
...
from .events import (
    ProductItemCreatedEvent,
    ProductItemUpdatedEvent,
    ProductItemStatusChangedEvent,
    ProductItemSubItem1AddedEvent,
    ProductItemSubItem2AddedEvent,
    ProductItemDeletedEvent
)

class ProductVariant(BaseModel):
    """Child Entity 1 of ProductItem Aggregate."""
...
class ProductAttribute(BaseModel):
    """Child Entity 2 of ProductItem Aggregate."""
    id: str = Field(default_factory=lambda: str(uuid.uuid4()))
    label: str
    value_payload: str
    score: float = 1.0
...
class ProductItemAggregate(BaseModel):
    """
    Aggregate Root for Product Catalog Service.
    ...
    sub_items_1: List[ProductVariant] = Field(default_factory=list)
    sub_items_2: List[ProductAttribute] = Field(default_factory=list)
    ...
    def validate_invariants(self) -> None:
        if not self.name or len(self.name.strip()) < 2:
            raise ProductItemValidationException("name", "Name must contain at least 2 characters")
        if not self.code or len(self.code.strip()) < 2:
            raise ProductItemValidationException("code", "Code must contain at least 2 characters")
        if len(self.sub_items_1) > 100:
            raise ProductItemQuotaExceededException("ProductVariant", 100)
        if len(self.sub_items_2) > 200:
            raise ProductItemQuotaExceededException("ProductAttribute", 200)
    ...
    def transition_status(self, target_status: str, actor_id: str, reason: Optional[str] = None) -> None:
        ...
        if target_status.upper() not in allowed:
            raise InvalidProductItemStateTransitionException(self.status, target_status.upper())
        ...
        self._pending_events.append(ProductItemStatusChangedEvent.create(
    ...
    def update_attributes(self, name: Optional[str], description: Optional[str], attributes: Optional[Dict[str, Any]], actor_id: str) -> None:
        ...
        if delta:
            self.version += 1
            self.updated_at = datetime.now(timezone.utc)
            self._pending_events.append(ProductItemUpdatedEvent.create(
                aggregate_id=self.id,
                version=self.version,
                changes=delta
            ))

    def add_sub_item_1(self, item: ProductVariant, actor_id: str) -> None:
        if any(x.code == item.code for x in self.sub_items_1):
            raise ProductItemValidationException("ProductVariant.code", f"Code '{item.code}' already exists in aggregate")
        self.sub_items_1.append(item)
        self.version += 1
        self.updated_at = datetime.now(timezone.utc)
        self._pending_events.append(ProductItemSubItem1AddedEvent.create(self.id, item.id))

    def add_sub_item_2(self, item: ProductAttribute, actor_id: str) -> None:
        self.sub_items_2.append(item)
        self.version += 1
        self.updated_at = datetime.now(timezone.utc)
        self._pending_events.append(ProductItemSubItem2AddedEvent.create(self.id, item.id))
    ...
    def soft_delete(self, actor_id: str) -> None:
        ...
        self.version += 1
        self.updated_at = datetime.now(timezone.utc)
        self._pending_events.append(ProductItemDeletedEvent.create(self.id, deleted_by=actor_id))
