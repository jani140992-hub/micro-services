...
async def test_catalog_service_repo_crud_operations():
    ...
    entity = ProductItemAggregate(name="Repository Record", code="REPO-001")
...
@pytest.mark.asyncio
async def test_catalog_service_repo_query_and_pagination():
    repo = ProductItemRepository()
    for i in range(25):
        await repo.save(ProductItemAggregate(
            name=f"Bulk Item {i}",
            code=f"BULK-{i:03d}",
