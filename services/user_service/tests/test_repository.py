"""Repository Unit Tests for User Profile Service."""
...
@pytest.mark.asyncio
async def test_user_service_repo_crud_operations():
    repo = UserProfileRepository()
    entity = UserProfileAggregate(name="Repository Record", code="REPO-001")
...
@pytest.mark.asyncio
async def test_user_service_repo_query_and_pagination():
    repo = UserProfileRepository()
    for i in range(25):
        await repo.save(UserProfileAggregate(
            name=f"Bulk Item {i}",
            code=f"BULK-{i:03d}",
