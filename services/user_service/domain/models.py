"""Domain Models and Aggregate Root for User Profile Service."""
...
from .events import (
    ...
    UserProfileStatusChangedEvent,
    UserProfileSubItem1AddedEvent,
    UserProfileSubItem2AddedEvent,
    UserProfileDeletedEvent
)

class CustomerAddress(BaseModel):
    """Child Entity 1 of UserProfile Aggregate."""
...
class UserPreference(BaseModel):
    """Child Entity 2 of UserProfile Aggregate."""
    id: str = Field(default_factory=lambda: str(uuid.uuid4()))
    label: str
    value_payload: str
    score: float = 1.0
...
class UserProfileAggregate(BaseModel):
    """
    Aggregate Root for User Profile Service.
    ...
    sub_items_1: List[CustomerAddress] = Field(default_factory=list)
    sub_items_2: List[UserPreference] = Field(default_factory=list)
    ...
    def validate_invariants(self) -> None:
        if not self.name or len(self.name.strip()) < 2:
            raise UserProfileValidationException("name", "Name must contain at least 2 characters")
        if not self.code or len(self.code.strip()) < 2:
            raise UserProfileValidationException("code", "Code must contain at least 2 characters")
        if len(self.sub_items_1) > 100:
            raise UserProfileQuotaExceededException("CustomerAddress", 100)
        if len(self.sub_items_2) > 200:
            raise UserProfileQuotaExceededException("UserPreference", 200)
    ...
    def transition_status(self, target_status: str, actor_id: str, reason: Optional[str] = None) -> None:
        ...
        if target_status.upper() not in allowed:
            raise InvalidUserProfileStateTransitionException(self.status, target_status.upper())
        ...
        self._pending_events.append(UserProfileStatusChangedEvent.create(
    ...
    def update_attributes(self, name: Optional[str], description: Optional[str], attributes: Optional[Dict[str, Any]], actor_id: str) -> None:
        ...
        if delta:
            self.version += 1
            self.updated_at = datetime.now(timezone.utc)
            self._pending_events.append(UserProfileUpdatedEvent.create(
                aggregate_id=self.id,
                version=self.version,
                changes=delta
            ))

    def add_sub_item_1(self, item: CustomerAddress, actor_id: str) -> None:
        if any(x.code == item.code for x in self.sub_items_1):
            raise UserProfileValidationException("CustomerAddress.code", f"Code '{item.code}' already exists in aggregate")
        self.sub_items_1.append(item)
        self.version += 1
        self.updated_at = datetime.now(timezone.utc)
        self._pending_events.append(UserProfileSubItem1AddedEvent.create(self.id, item.id))

    def add_sub_item_2(self, item: UserPreference, actor_id: str) -> None:
        self.sub_items_2.append(item)
        self.version += 1
        self.updated_at = datetime.now(timezone.utc)
        self._pending_events.append(UserProfileSubItem2AddedEvent.create(self.id, item.id))
    ...
    def soft_delete(self, actor_id: str) -> None:
        ...
        self.version += 1
        self.updated_at = datetime.now(timezone.utc)
        self._pending_events.append(UserProfileDeletedEvent.create(self.id, deleted_by=actor_id))
