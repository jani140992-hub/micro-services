"""Event Publisher for User Profile Service."""
...
logger = logging.getLogger("user_service.events.producer")

class UserProfileEventProducer:
    """Publishes events from User Profile Service to Kafka or In-Memory Bus."""
    ...
    async def publish(self, event: Any) -> bool:
        if isinstance(event, EventBase):
            logger.info(f"[user_service] Publishing {event.event_type} (id={event.event_id})")
