...
class UserProfileRepository:
    ...
    def __init__(self) -> None:
        self._storage: Dict[str, UserProfileAggregate] = {}
        self._code_index: Dict[str, str] = {}

    async def get_by_id(self, entity_id: str) -> Optional[UserProfileAggregate]:
        entity = self._storage.get(entity_id)
        if entity and not entity.is_deleted:
            return entity
        return None

    async def get_by_code(self, code: str) -> Optional[UserProfileAggregate]:
        eid = self._code_index.get(code.upper())
        if eid:
            return await self.get_by_id(eid)
        return None

    async def save(self, entity: UserProfileAggregate) -> UserProfileAggregate:
    ...
    async def query(
        ...
        skip: int = 0,
        limit: int = 20
    ) -> Tuple[List[UserProfileAggregate], int]:
