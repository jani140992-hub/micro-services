"""Domain and Persistence Entity Mappers for User Profile Service."""

import logging
from typing import Any, Dict, List, Optional
from services.user_service.domain.models import UserProfileAggregate, CustomerAddress, UserPreference
from services.user_service.repositories.models import UserProfileORM, CustomerAddressORM, UserPreferenceORM
from services.user_service.dto.responses import UserProfileDetailResponse, UserProfileSummaryResponse

logger = logging.getLogger("user_service.mappers")

class UserProfileDataMapper:
    """Bi-directional mapping between domain aggregates and database models."""
    @staticmethod
    def to_orm(aggregate: UserProfileAggregate) -> UserProfileORM:
        orm = UserProfileORM(
            id=aggregate.id,
    ...
    @staticmethod
    def to_domain(orm: UserProfileORM) -> UserProfileAggregate:
...
class ContextProjectionMapper01:
    ...
    def transform(self, aggregate: UserProfileAggregate) -> Dict[str, Any]:
        return {
