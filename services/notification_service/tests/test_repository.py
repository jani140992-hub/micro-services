...
async def test_notification_service_repo_crud_operations():
    ...
    entity = NotificationMessageAggregate(name="Repository Record", code="REPO-001")
...
@pytest.mark.asyncio
async def test_notification_service_repo_query_and_pagination():
    repo = NotificationMessageRepository()
    for i in range(25):
        await repo.save(NotificationMessageAggregate(
            name=f"Bulk Item {i}",
            code=f"BULK-{i:03d}",
