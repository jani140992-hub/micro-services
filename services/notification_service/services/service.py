"""Application Service Orchestrator for Notification & Messaging Service."""
...
class NotificationMessageService:
    ...
    def _to_summary(self, entity: NotificationMessageAggregate) -> NotificationMessageSummaryResponse:
        return NotificationMessageSummaryResponse(
            id=entity.id,
    ...
    def _to_detail(self, entity: NotificationMessageAggregate) -> NotificationMessageDetailResponse:
        return NotificationMessageDetailResponse(
            id=entity.id,
            tenant_id=entity.tenant_id,
            name=entity.name,
            code=entity.code,
            ...
            sub_items_1=[DeliveryAttemptRecordResponse(**item.model_dump()) for item in entity.sub_items_1],
            sub_items_2=[TemplateVariableResponse(**item.model_dump()) for item in entity.sub_items_2],
    ...
    async def create(self, req: CreateNotificationMessageRequest, actor_id: str = "system") -> NotificationMessageDetailResponse:
        existing = await self.repository.get_by_code(req.code)
        if existing:
            raise NotificationMessageAlreadyExistsException(req.code)

        entity = NotificationMessageAggregate(
            name=req.name,
            code=req.code,
        ...
        logger.info(f"Successfully created NotificationMessage {entity.id} [code={entity.code}]")
        return self._to_detail(entity)

    async def get_by_id(self, entity_id: str) -> NotificationMessageDetailResponse:
        cached = await self.cache.get(entity_id)
        if cached:
            return NotificationMessageDetailResponse(**cached)

        entity = await self.repository.get_by_id(entity_id)
        if not entity:
            raise NotificationMessageNotFoundException(entity_id)

        await self.cache.set(entity)
        return self._to_detail(entity)

    async def update(self, entity_id: str, req: UpdateNotificationMessageRequest, actor_id: str = "system") -> NotificationMessageDetailResponse:
        entity = await self.repository.get_by_id(entity_id)
        if not entity:
            raise NotificationMessageNotFoundException(entity_id)
        ...
        logger.info(f"Updated NotificationMessage {entity.id}")
        return self._to_detail(entity)

    async def change_status(self, entity_id: str, req: ChangeNotificationMessageStatusRequest, actor_id: str = "system") -> NotificationMessageDetailResponse:
        entity = await self.repository.get_by_id(entity_id)
        if not entity:
            raise NotificationMessageNotFoundException(entity_id)
        ...
        logger.info(f"Status transition on NotificationMessage {entity.id} -> {req.target_status}")
        return self._to_detail(entity)

    async def add_sub_item_1(self, entity_id: str, req: AddDeliveryAttemptRecordRequest, actor_id: str = "system") -> NotificationMessageDetailResponse:
        entity = await self.repository.get_by_id(entity_id)
        if not entity:
            raise NotificationMessageNotFoundException(entity_id)

        item = DeliveryAttemptRecord(
            name=req.name,
            code=req.code,
    ...
    async def add_sub_item_2(self, entity_id: str, req: AddTemplateVariableRequest, actor_id: str = "system") -> NotificationMessageDetailResponse:
        entity = await self.repository.get_by_id(entity_id)
        if not entity:
            raise NotificationMessageNotFoundException(entity_id)
    ...
    async def remove_sub_item_1(self, entity_id: str, sub1_id: str) -> NotificationMessageDetailResponse:
        entity = await self.repository.get_by_id(entity_id)
        if not entity:
            raise NotificationMessageNotFoundException(entity_id)
    ...
    async def remove_sub_item_2(self, entity_id: str, sub2_id: str) -> NotificationMessageDetailResponse:
        entity = await self.repository.get_by_id(entity_id)
        if not entity:
            raise NotificationMessageNotFoundException(entity_id)
    ...
    async def delete(self, entity_id: str, actor_id: str = "system") -> bool:
        ...
        logger.info(f"Soft deleted NotificationMessage {entity_id}")
        return True

    async def query_page(self, q: QueryNotificationMessageRequest) -> NotificationMessagePageResponse:
        ...
        return NotificationMessagePageResponse(
    ...
    async def batch_action(self, batch_req: BatchNotificationMessageActionRequest, actor_id: str = "system") -> BatchActionResultResponse:
        success: List[str] = []
        errors: Dict[str, str] = {}
        ...
        for eid in batch_req.entity_ids:
            try:
                if batch_req.action == "ACTIVATE":
                    await self.change_status(eid, ChangeNotificationMessageStatusRequest(target_status="ACTIVE", reason=batch_req.reason), actor_id)
                elif batch_req.action == "SUSPEND":
                    await self.change_status(eid, ChangeNotificationMessageStatusRequest(target_status="SUSPENDED", reason=batch_req.reason), actor_id)
                elif batch_req.action == "ARCHIVE":
                    await self.change_status(eid, ChangeNotificationMessageStatusRequest(target_status="ARCHIVED", reason=batch_req.reason), actor_id)
