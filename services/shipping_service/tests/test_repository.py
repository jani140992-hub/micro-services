"""Repository Unit Tests for Shipping & Logistics Service."""
...
@pytest.mark.asyncio
async def test_shipping_service_repo_crud_operations():
    repo = ShipmentConsignmentRepository()
    entity = ShipmentConsignmentAggregate(name="Repository Record", code="REPO-001")
...
@pytest.mark.asyncio
async def test_shipping_service_repo_query_and_pagination():
    repo = ShipmentConsignmentRepository()
    for i in range(25):
        await repo.save(ShipmentConsignmentAggregate(
            name=f"Bulk Item {i}",
            code=f"BULK-{i:03d}",
