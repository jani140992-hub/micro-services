"""Application Service Orchestrator for Inventory Management Service."""
...
class StockItemService:
    ...
    def _to_summary(self, entity: StockItemAggregate) -> StockItemSummaryResponse:
        return StockItemSummaryResponse(
            id=entity.id,
    ...
    def _to_detail(self, entity: StockItemAggregate) -> StockItemDetailResponse:
        return StockItemDetailResponse(
            ...
            sub_items_1=[WarehouseLocationResponse(**item.model_dump()) for item in entity.sub_items_1],
            sub_items_2=[StockReservationResponse(**item.model_dump()) for item in entity.sub_items_2],
    ...
    async def create(self, req: CreateStockItemRequest, actor_id: str = "system") -> StockItemDetailResponse:
        existing = await self.repository.get_by_code(req.code)
        if existing:
            raise StockItemAlreadyExistsException(req.code)

        entity = StockItemAggregate(
            name=req.name,
            code=req.code,
        ...
        logger.info(f"Successfully created StockItem {entity.id} [code={entity.code}]")
        return self._to_detail(entity)

    async def get_by_id(self, entity_id: str) -> StockItemDetailResponse:
        cached = await self.cache.get(entity_id)
        if cached:
            return StockItemDetailResponse(**cached)
    ...
    async def update(self, entity_id: str, req: UpdateStockItemRequest, actor_id: str = "system") -> StockItemDetailResponse:
        entity = await self.repository.get_by_id(entity_id)
        if not entity:
            raise StockItemNotFoundException(entity_id)
        ...
        logger.info(f"Updated StockItem {entity.id}")
        return self._to_detail(entity)

    async def change_status(self, entity_id: str, req: ChangeStockItemStatusRequest, actor_id: str = "system") -> StockItemDetailResponse:
        entity = await self.repository.get_by_id(entity_id)
        if not entity:
            raise StockItemNotFoundException(entity_id)
        ...
        logger.info(f"Status transition on StockItem {entity.id} -> {req.target_status}")
        return self._to_detail(entity)

    async def add_sub_item_1(self, entity_id: str, req: AddWarehouseLocationRequest, actor_id: str = "system") -> StockItemDetailResponse:
        entity = await self.repository.get_by_id(entity_id)
        if not entity:
            raise StockItemNotFoundException(entity_id)
    ...
    async def add_sub_item_2(self, entity_id: str, req: AddStockReservationRequest, actor_id: str = "system") -> StockItemDetailResponse:
        entity = await self.repository.get_by_id(entity_id)
        if not entity:
            raise StockItemNotFoundException(entity_id)
    ...
    async def remove_sub_item_1(self, entity_id: str, sub1_id: str) -> StockItemDetailResponse:
        entity = await self.repository.get_by_id(entity_id)
        if not entity:
            raise StockItemNotFoundException(entity_id)
    ...
    async def remove_sub_item_2(self, entity_id: str, sub2_id: str) -> StockItemDetailResponse:
        entity = await self.repository.get_by_id(entity_id)
        if not entity:
            raise StockItemNotFoundException(entity_id)
    ...
    async def delete(self, entity_id: str, actor_id: str = "system") -> bool:
        ...
        logger.info(f"Soft deleted StockItem {entity_id}")
        return True

    async def query_page(self, q: QueryStockItemRequest) -> StockItemPageResponse:
    ...
    async def batch_action(self, batch_req: BatchStockItemActionRequest, actor_id: str = "system") -> BatchActionResultResponse:
        success: List[str] = []
        errors: Dict[str, str] = {}
        ...
        for eid in batch_req.entity_ids:
            try:
                if batch_req.action == "ACTIVATE":
                    await self.change_status(eid, ChangeStockItemStatusRequest(target_status="ACTIVE", reason=batch_req.reason), actor_id)
                elif batch_req.action == "SUSPEND":
                    await self.change_status(eid, ChangeStockItemStatusRequest(target_status="SUSPENDED", reason=batch_req.reason), actor_id)
                elif batch_req.action == "ARCHIVE":
                    await self.change_status(eid, ChangeStockItemStatusRequest(target_status="ARCHIVED", reason=batch_req.reason), actor_id)
