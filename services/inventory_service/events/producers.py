"""Event Publisher for Inventory Management Service."""
...
logger = logging.getLogger("inventory_service.events.producer")

class StockItemEventProducer:
    """Publishes events from Inventory Management Service to Kafka or In-Memory Bus."""
    ...
    async def publish(self, event: Any) -> bool:
        if isinstance(event, EventBase):
            logger.info(f"[inventory_service] Publishing {event.event_type} (id={event.event_id})")
