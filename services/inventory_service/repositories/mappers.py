"""Domain and Persistence Entity Mappers for Inventory Management Service."""

import logging
from typing import Any, Dict, List, Optional
from services.inventory_service.domain.models import StockItemAggregate, WarehouseLocation, StockReservation
from services.inventory_service.repositories.models import StockItemORM, WarehouseLocationORM, StockReservationORM
from services.inventory_service.dto.responses import StockItemDetailResponse, StockItemSummaryResponse

logger = logging.getLogger("inventory_service.mappers")

class StockItemDataMapper:
    """Bi-directional mapping between domain aggregates and database models."""
    @staticmethod
    def to_orm(aggregate: StockItemAggregate) -> StockItemORM:
        orm = StockItemORM(
            id=aggregate.id,
    ...
    @staticmethod
    def to_domain(orm: StockItemORM) -> StockItemAggregate:
        return StockItemAggregate(
            id=orm.id,
...
class ContextProjectionMapper01:
    ...
    def transform(self, aggregate: StockItemAggregate) -> Dict[str, Any]:
        return {
