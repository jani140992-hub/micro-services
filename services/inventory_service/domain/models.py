"""Domain Models and Aggregate Root for Inventory Management Service."""
...
from .events import (
    ...
    StockItemStatusChangedEvent,
    StockItemSubItem1AddedEvent,
    StockItemSubItem2AddedEvent,
    StockItemDeletedEvent
)

class WarehouseLocation(BaseModel):
    """Child Entity 1 of StockItem Aggregate."""
...
class StockReservation(BaseModel):
    """Child Entity 2 of StockItem Aggregate."""
    id: str = Field(default_factory=lambda: str(uuid.uuid4()))
    label: str
    value_payload: str
    score: float = 1.0
...
class StockItemAggregate(BaseModel):
    """
    Aggregate Root for Inventory Management Service.
    ...
    sub_items_1: List[WarehouseLocation] = Field(default_factory=list)
    sub_items_2: List[StockReservation] = Field(default_factory=list)
    ...
    def validate_invariants(self) -> None:
        if not self.name or len(self.name.strip()) < 2:
            raise StockItemValidationException("name", "Name must contain at least 2 characters")
        if not self.code or len(self.code.strip()) < 2:
            raise StockItemValidationException("code", "Code must contain at least 2 characters")
        if len(self.sub_items_1) > 100:
            raise StockItemQuotaExceededException("WarehouseLocation", 100)
        if len(self.sub_items_2) > 200:
            raise StockItemQuotaExceededException("StockReservation", 200)
    ...
    def transition_status(self, target_status: str, actor_id: str, reason: Optional[str] = None) -> None:
        ...
        if target_status.upper() not in allowed:
            raise InvalidStockItemStateTransitionException(self.status, target_status.upper())
        ...
        self._pending_events.append(StockItemStatusChangedEvent.create(
    ...
    def update_attributes(self, name: Optional[str], description: Optional[str], attributes: Optional[Dict[str, Any]], actor_id: str) -> None:
        ...
        if delta:
            self.version += 1
            self.updated_at = datetime.now(timezone.utc)
            self._pending_events.append(StockItemUpdatedEvent.create(
                aggregate_id=self.id,
                version=self.version,
                changes=delta
            ))

    def add_sub_item_1(self, item: WarehouseLocation, actor_id: str) -> None:
        if any(x.code == item.code for x in self.sub_items_1):
            raise StockItemValidationException("WarehouseLocation.code", f"Code '{item.code}' already exists in aggregate")
        self.sub_items_1.append(item)
        self.version += 1
        self.updated_at = datetime.now(timezone.utc)
        self._pending_events.append(StockItemSubItem1AddedEvent.create(self.id, item.id))

    def add_sub_item_2(self, item: StockReservation, actor_id: str) -> None:
        self.sub_items_2.append(item)
        self.version += 1
        self.updated_at = datetime.now(timezone.utc)
        self._pending_events.append(StockItemSubItem2AddedEvent.create(self.id, item.id))
    ...
    def soft_delete(self, actor_id: str) -> None:
        ...
        self.version += 1
        self.updated_at = datetime.now(timezone.utc)
        self._pending_events.append(StockItemDeletedEvent.create(self.id, deleted_by=actor_id))
