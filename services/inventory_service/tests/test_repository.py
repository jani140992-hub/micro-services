...
async def test_inventory_service_repo_crud_operations():
    ...
    entity = StockItemAggregate(name="Repository Record", code="REPO-001")
...
@pytest.mark.asyncio
async def test_inventory_service_repo_query_and_pagination():
    repo = StockItemRepository()
    for i in range(25):
        await repo.save(StockItemAggregate(
            name=f"Bulk Item {i}",
            code=f"BULK-{i:03d}",
