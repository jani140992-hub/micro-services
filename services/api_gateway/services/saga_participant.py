"""Distributed Saga Participant and Compensation Logic for API Gateway Service."""

import logging
from datetime import datetime, timezone
from typing import Any, Dict, List, Optional
from services.api_gateway.services.service import GatewayRouteService
from services.api_gateway.dto.requests import ChangeGatewayRouteStatusRequest

logger = logging.getLogger("api_gateway.saga")

class GatewayRouteSagaParticipant:
    """Coordinates distributed transaction steps and compensating rollbacks."""
    def __init__(self, service: GatewayRouteService) -> None:
    ...
    async def prepare_step(self, saga_id: str, step_name: str, entity_id: str, payload: Dict[str, Any]) -> bool:
        logger.info(f"Saga {saga_id} PREPARE step {step_name} on api_gateway {entity_id}")
    ...
    async def commit_step(self, saga_id: str) -> bool:
        ...
        if not record:
            ...
            return False
        logger.info(f"Saga {saga_id} COMMITTING step {record['step_name']} on api_gateway")
    ...
    async def compensate_step(self, saga_id: str, failure_reason: str) -> bool:
        ...
        try:
            await self.service.change_status(entity_id, ChangeGatewayRouteStatusRequest(
...
class SagaWorkflowStep01:
    ...
    def __init__(self, participant: GatewayRouteSagaParticipant) -> None:
        self.participant = participant
        self.step_code = "STEP_API_001"
