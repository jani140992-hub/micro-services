...
async def test_api_gateway_repo_crud_operations():
    ...
    entity = GatewayRouteAggregate(name="Repository Record", code="REPO-001")
...
@pytest.mark.asyncio
async def test_api_gateway_repo_query_and_pagination():
    repo = GatewayRouteRepository()
    for i in range(25):
        await repo.save(GatewayRouteAggregate(
            name=f"Bulk Item {i}",
            code=f"BULK-{i:03d}",
