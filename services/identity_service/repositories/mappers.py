"""Domain and Persistence Entity Mappers for Identity & Authentication Service."""

import logging
from typing import Any, Dict, List, Optional
from services.identity_service.domain.models import UserCredentialAggregate, UserSession, RoleAssignment
from services.identity_service.repositories.models import UserCredentialORM, UserSessionORM, RoleAssignmentORM
from services.identity_service.dto.responses import UserCredentialDetailResponse, UserCredentialSummaryResponse

logger = logging.getLogger("identity_service.mappers")

class UserCredentialDataMapper:
    """Bi-directional mapping between domain aggregates and database models."""
    @staticmethod
    def to_orm(aggregate: UserCredentialAggregate) -> UserCredentialORM:
        orm = UserCredentialORM(
            id=aggregate.id,
    ...
    @staticmethod
    def to_domain(orm: UserCredentialORM) -> UserCredentialAggregate:
        return UserCredentialAggregate(
            id=orm.id,
...
class ContextProjectionMapper01:
    ...
    def transform(self, aggregate: UserCredentialAggregate) -> Dict[str, Any]:
        return {
