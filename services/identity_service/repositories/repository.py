...
class UserCredentialRepository:
    ...
    def __init__(self) -> None:
        self._storage: Dict[str, UserCredentialAggregate] = {}
        self._code_index: Dict[str, str] = {}

    async def get_by_id(self, entity_id: str) -> Optional[UserCredentialAggregate]:
        entity = self._storage.get(entity_id)
        if entity and not entity.is_deleted:
            return entity
        return None

    async def get_by_code(self, code: str) -> Optional[UserCredentialAggregate]:
        eid = self._code_index.get(code.upper())
        if eid:
            return await self.get_by_id(eid)
        return None

    async def save(self, entity: UserCredentialAggregate) -> UserCredentialAggregate:
    ...
    async def query(
        ...
        skip: int = 0,
        limit: int = 20
    ) -> Tuple[List[UserCredentialAggregate], int]:
