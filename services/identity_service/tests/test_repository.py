...
async def test_identity_service_repo_crud_operations():
    ...
    entity = UserCredentialAggregate(name="Repository Record", code="REPO-001")
...
@pytest.mark.asyncio
async def test_identity_service_repo_query_and_pagination():
    repo = UserCredentialRepository()
    for i in range(25):
        await repo.save(UserCredentialAggregate(
            name=f"Bulk Item {i}",
            code=f"BULK-{i:03d}",
