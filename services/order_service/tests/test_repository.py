...
async def test_order_service_repo_crud_operations():
    ...
    entity = CustomerOrderAggregate(name="Repository Record", code="REPO-001")
...
@pytest.mark.asyncio
async def test_order_service_repo_query_and_pagination():
    repo = CustomerOrderRepository()
    for i in range(25):
        await repo.save(CustomerOrderAggregate(
            name=f"Bulk Item {i}",
            code=f"BULK-{i:03d}",
