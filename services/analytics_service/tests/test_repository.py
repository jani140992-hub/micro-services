...
async def test_analytics_service_repo_crud_operations():
    ...
    entity = StreamMetricRecordAggregate(name="Repository Record", code="REPO-001")
...
@pytest.mark.asyncio
async def test_analytics_service_repo_query_and_pagination():
    repo = StreamMetricRecordRepository()
    for i in range(25):
        await repo.save(StreamMetricRecordAggregate(
            name=f"Bulk Item {i}",
            code=f"BULK-{i:03d}",
