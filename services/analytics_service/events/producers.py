"""Event Publisher for Analytics & BI Service."""
...
logger = logging.getLogger("analytics_service.events.producer")

class StreamMetricRecordEventProducer:
    """Publishes events from Analytics & BI Service to Kafka or In-Memory Bus."""
    ...
    async def publish(self, event: Any) -> bool:
        if isinstance(event, EventBase):
            logger.info(f"[analytics_service] Publishing {event.event_type} (id={event.event_id})")
